"""Data management and synchronization for Sure Finance addon.

This module handles data fetching, caching, and synchronization
between the Sure Finance API and Home Assistant.
"""
...
class DataManager:
    ...
    def __init__(
        ...
        Args:
            api_client: Sure Finance API client
            cache_manager: Cache manager instance
            calculator: Financial calculator instance
    ...
    async def get_transactions(
        ...
        """Get transactions for period with caching.
    ...
    async def get_categories(self, force_refresh: bool = False) -> List[Category]:
        ...
        try:
            ...
            all_categories = await self.api_client.get_all_pages(
    ...
    async def get_merchants(self, force_refresh: bool = False) -> List[Merchant]:
        """Get all merchants with caching.
        ...
        try:
            ...
            # Cache for longer as merchants don't change often
            await self.cache.set(
                cache_key,
                merchants,
                ttl=86400,  # 24 hours
    ...
    async def get_tags(self, force_refresh: bool = False) -> List[Tag]:
        ...
        try:
            ...
            # Cache for longer as tags don't change often
            await self.cache.set(
                cache_key,
                tags,
                ttl=86400,  # 24 hours
    ...
    async def get_monthly_cashflow(
        ...
        Returns:
            Cashflow summary for the month
        """
        ...
        date_range = DateRangeParams(start_date=start_date, end_date=end_date)
        transactions = await self.api_client.get_all_pages(
            self.api_client.get_transactions,
            per_page=100,
            date_range=date_range
        )
        ...
        summary = self.calculator.calculate_cashflow_summary(
            transaction_models,
            start_date,
            end_date
        )
    ...
    async def sync_all_data(self):
        ...
        try:
            ...
            # Then sync transactions (may depend on accounts)
            await self.get_transactions(days=90, force_refresh=True)
            
            # Finally calculate summaries
